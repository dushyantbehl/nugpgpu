...
static void nugpgpu_check_stolen(struct nugpgpu_private *gpu_priv)
{
  struct resource *r;
  u32 base;

  pci_read_config_dword(gpu_priv->pdev, 0x5c, &base);
  base &= ~((1<<20) - 1);
  printk(LOG_INFO "base is 0x%x\n" LOG_END,(unsigned)base);

    
  r = devm_request_mem_region(gpu_priv->dev_device, base, gpu_priv->gtt.stolen_size, 
            "Graphics Stolen Memory");
  if (r == NULL)
    printk(LOG_ERR "Someome else has access to the stolen mem\n" LOG_END);
}
...
int nugpgpu_gtt_init(struct nugpgpu_private *gpu_priv)
{
  ...
  nugpgpu_check_stolen(gpu_priv);

  if(IS_PAGE_FAULT_ENABLE)
      printk(LOG_INFO "page faults enabled\n" LOG_END);
                  ...
                  unsigned long *mappable_end)
{
  ...
  *mappable_end = pci_resource_len(gpu_priv->pdev, 2);
  ...
  pci_read_config_word(gpu_priv->pdev, HSW_GMCH_CTRL, &hsw_gmch_ctl);
  printk(LOG_INFO "GSA_CR_MGGC0_2_0_PCI = 0x%hx\n" LOG_END, hsw_gmch_ctl);

  // Stolen Size / GTT Graphics Memory Size / GGMS (9:8)
  stolen_gmch = hsw_gmch_ctl >> HSW_GMCH_GMS_SHIFT;
  stolen_gmch &= HSW_GMCH_GMS_MASK;
  *stolen_size = stolen_gmch << HSW_GMCH_GMS_REVSHIFT;
  ...
  total_gmch = hsw_gmch_ctl >> HSW_GMCH_GGMS_SHIFT;
  total_gmch &= HSW_GMCH_GGMS_MASK;
  gtt_size = total_gmch << HSW_GMCH_GGMS_REVSHIFT;
  ...
  *total_size = (gtt_size / sizeof(nugpgpu_gtt_pte_t)) << PAGE_SHIFT;
  ...
  if (!gpu_priv->gtt.gsm) {
    printk(LOG_INFO "Mapping of GTT page table has failed, You're doomed\n" LOG_END);
  }
                                        ...
                                        uint32_t level)
{
  ...
  //Flush the GPU TLB.
  NUGPGPU_WRITE(GFX_FLSH_CNTL_HSW, GFX_FLSH_CNTL_EN);
  NUGPGPU_READ(GFX_FLSH_CNTL_HSW);
...
static int scratchpage_setup(struct nugpgpu_private *gpu_priv)
{
  ...
  printk(LOG_INFO "PCI map page successful\n" LOG_END);
